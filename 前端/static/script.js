...
async function loadTimeline(){
  ...
  // 千分位與收盤文字（盤中顯示 ─（盤中））
  const fmt = (x) => (x == null ? '-' : Number(x).toLocaleString('zh-TW'));
  const closeTxt = (m.close == null) ? '—（盤中）' : fmt(m.close);

  // ✅ 改為中文資訊列 +「間隔」
  document.getElementById('itdMeta').textContent =
    `開盤價 ${fmt(m.open)} ｜ 收盤價 ${closeTxt} ｜ 最高點 ${fmt(m.high)} ｜ 最低點 ${fmt(m.low)} ｜ 間隔 ${m.step} 分 ｜ 筆數 ${m.count}`;
  ...
  if (mode) mode.textContent = `模式：概覽（間隔 ${ITD_STEP} 分）`;
}
...
function renderTimeline(){
  ...
  _tl.forEach((p) => {
    ...
    else icon = (p.dir === 'up') ? '+' : (p.dir === 'down' ? '−' : '±'); // U+2212 負號
